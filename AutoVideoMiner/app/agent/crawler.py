...
class CrawlerAgent(AgentMemoryRuntime):
    ...
    def crawl(self, platform: str, keyword: str, task_mode: str = "probe", short_memory: dict[str, Any] | None = None, logs_dir: str = "") -> tuple[list[dict], dict[str, Any]]:
        m = self._init_memory("crawler_agent", short_memory)
        m["system_prompt"] = get_prompt("crawler", "system_main")
        self._append_memory(m, f"CRAWL:{platform}:{keyword}:{task_mode}")
