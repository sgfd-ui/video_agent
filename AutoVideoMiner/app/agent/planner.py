...
class PlannerAgent(AgentMemoryRuntime):
    ...
    def plan(self, target_scene: str, event_name: str | None = None) -> PlanResult:
        ...
        global_rules_path, scene_md_path = manager.ensure_scene_memory_file(scene)
        manager.insert_task_anchor(scene_md_path, task_id)
        LOGGER.info("planner start: scene=%s task_id=%s scene_md=%s", scene, task_id, scene_md_path)
