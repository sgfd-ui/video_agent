...
class EvaluatorAgent(AgentMemoryRuntime):
    ...
    def evaluate(self, platform: str, keyword: str, top_5_results: list[dict], target_scene: str, token_usage: dict[str, Any] | None = None, short_memory: dict[str, Any] | None = None, logs_dir: str = "") -> tuple[float, str, dict[str, Any]]:
        m = self._init_memory("evaluator_agent", short_memory)
        m["system_prompt"] = get_prompt("evaluator", "system_main")
        ...
        self._append_memory(m, f"EVAL:{platform}:{keyword}:{score}")
        if logs_dir:
            self._compact_if_needed(logs_dir, m)
        return max(0.0, min(1.0, score)), reason, m
